...
def _train_generator(self, classifier_ids, label_probs, classifier_params: List[NDArrays]):
    with self.mlflow.start_run(run_id=self.server_run_id):
        ...
        for epoch in range(num_epochs):
            ...
            pi = dirichlet.sample()  # Sample probability vector
            categorical = Categorical(pi)
            labels = categorical.sample((batch_size,)).to(self.device)  # Sample labels
